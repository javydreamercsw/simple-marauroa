...
---
This is how new functionality is MEANT to be coded. This is the correct way to extend the simple server, or Marauroa for that matter. When possible use a server extension so others can reuse your code without modifying their code (only the clients might need to be modified just to react to the new functionality like actions and events).

To accomplish that I did the following:
* Moved RoomEvent to its respective simple server packages.
* Created RoomCRUDExtension class incorporating CUDRoomAction
* Added the LIST operation (see update comment)
