...
class GroupsXMLLoader extends DefaultHandler {
    ...
    private static final Logger LOG
            = Logger.getLogger(GroupsXMLLoader.class.getSimpleName());
    /**
     ...
     * <p>
     * Any exception occurring while examining a group value is ignored and
     * written to the loggers error channel
     *
     */
    @Override
        ...
        if (qName.equals("group")) {
            ...
            if (uriValue == null) {
                ...
                try {
                    ...
                    LOG.log(Level.SEVERE, "Invalid group reference: {0} [{1}]",
                            new Object[]{uriValue, ex.getMessage()});
                }

            }
        } else if (!qName.equals("groups")) {
            LOG.log(Level.WARNING, "Unknown XML element: {0}", qName);
        }
    }
}
